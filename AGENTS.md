...
- **One catalogue template.** `/products/[...filters]` (`src/pages/products/[...filters]/index.astro`) is the single catalogue page (path-based pre-filtering); do not re-add per-collection page templates — Phase-1 `/collections/[slug]` pages are retired with 301s (PRD R5).
