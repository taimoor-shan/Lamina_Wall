...
- **Recap table pattern** (the /request catalogue table): at build time the page bakes a `code → {thumbnail, collection}` JSON map (`getImage` with explicit `width: 120` — `widths:[120]` alone leaves `.src` at full size — plus `getCollection`, one 120w WebP per article, map keys are the products' uppercase `code`) into `<script type="application/json" id="recap-data">` whose **body** is injected with `set:html={JSON.stringify(recapData)}`. Astro never templates script bodies, so the JSON must be injected — and it must be a body, not a `data-value` attribute: attribute embedding entity-escapes every `"` as `&quot;`, which glued the `/_astro/…webp` names to their terminators and got the recap's own thumbnails pruned as "unreferenced" by the prune hook (R4 regression, 404s; the hook now entity-decodes before matching as a second line of defence). Runtime reads it via `(document.getElementById('recap-data') as HTMLScriptElement).textContent` + `JSON.parse` (note: the built tag drops `is:inline` and reorders attributes — `type` first). Rows are rendered by the inline JS into `<tbody id="recap-body">`; codes not in the map (retired pre-R1 codes left in an old tray) fall back to a blank tile + em dash. Headerless row layout: `[48px swatch] [code over collection] [×qty] [Remove]`, card grid ≤640px.
...
- **No host filesystem during prerender** — the Cloudflare adapter prerenders static routes inside a workerd sandbox (`process.cwd()` is `/bundle`, fs writes don't persist), so `fs.existsSync` in page frontmatter reads false even for files that exist. Build-time existence gates (e.g. the `public/catalogue.pdf` download CTA) must be computed in `astro.config.mjs` — which Astro loads from the project root — and injected via `vite.define` (see the `__R3_CATALOGUE_PDF__` pattern, PRD §10 Session 14). Changing the gated file requires a rebuild.
