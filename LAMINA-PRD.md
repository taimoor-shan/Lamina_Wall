...
### Changelog
**v2.1 → v2.2** (photography backfill, 2026-09-09):
- Client supplied real swatch photography for the remaining catalogue codes; the catalogue is now **258/258 real, 0 placeholders** (§10 Session 21). R1's "89 real / 169 placeholder" snapshot and §7 Q7 are superseded.
- **HGM-284A/B correction:** the manufacturer's index prints the Kanas Slate pair as 285A/B, but the client's uploaded photography (and the client's decision, Session 21) numbers them **284A/B** — the master's HGM-285 row is superseded by HGM-284 (`data/catalogue-master.ts`). `subcategory-taxonomy.md` is untouched (extraction record); the discrepancy is documented in §10 Session 21.
- AMF swatch images were uploaded under `src/assets/products/metal/`; moved to `src/assets/products/glossy-decorative/` to match the taxonomy's AMF family (Decorative & Mirror). The stale "silver"/"metal" `collection` values on the AMF product JSONs (which would have failed the build) were corrected by regeneration.
- `scripts/generate-catalogue.mjs` data-loss guard fixed: it counted placeholder refs (`placeholder-missing.png` exists on disk) as "previously real", so a run with any remaining placeholders falsely aborted. Guard now only counts non-placeholder swatchImage refs.
...
| Milestone | Status | Notes |
|---|---|---|
| R1 — Catalogue Data & Taxonomy | ☑ Done | 258 products (6 families / 19 series), **89 real / 169 placeholder at build — all 258 real / 0 placeholder since Session 21** (client backfilled the full photography set): `series`/`subcategory`/`imageStatus` schema, `data/catalogue-master.ts` + `scripts/generate-catalogue.mjs` + shared placeholder tile, collection titles updated (Wood / Stone & Marble / Metal / Textile / Solid Color / Decorative & Mirror). Build + tsc verified; 3 index-absent orphans (WG-37/WG-43/HGF-252) excluded & quarantined — §10 Session 12. |
...
**Files:** modified `astro.config.mjs` (Astro `redirects` map generated from the collections collection JSONs at config load + `build.redirects: false`); deleted `src/pages/collections/[slug].astro`, `AppShowcase.astro`, `CollectionSpread.astro`, `CollectionDialog.astro`, `IndexStrip.astro` (the Files line's "repurpose [slug].astro" wasn't literally possible — an existing page file outranks config redirects in route priority, so deletion is the mechanism's precondition; the outcome, collection-generated 301s in the asset bundle, matches the intent); modified `src/pages/sitemap.xml.ts` (27-route set derived from the collections collection + the SERIES taxonomy table), `src/components/Nav.astro` (chrome links → `/products/[family]`), `src/pages/products/[...filters]/index.astro` (pre-existing canonical bug: series pages canonicalized to their family URL — now depth-correct), `src/pages/style-guide.astro` (retired-component demo sections → a "Retired (R5)" note); comment-only `Seo.astro`, `content.config.ts`. No orphaned global styles existed (the four components were scoped).

- [x] `/collections/[slug]` → `/products/[family]` **301** — Astro config `redirects` → `@astrojs/cloudflare` emits Netlify-style rules into `dist/client/_redirects` at `astro:build:done` (from the route manifest via `@astrojs/underscore-redirects`); Workers Static Assets serves them at the asset layer as real HTTP 301s. `build.redirects: false` stops Astro also emitting 200-servable meta-refresh HTML stubs — a lost rule must 404 loudly. Map generated from the collections collection (same project-root-fs pattern as `__R3_CATALOGUE_PDF__`); no runtime map, no on-demand function.
- [x] Sitemap regenerated for the new routes (/, `/products`, `/products/[family]`, `/products/[family]/[series]`); canonical/OG/robots updated; `/request`, `/thank-you`, `/style-guide` stay `noindex`.
- [x] Remove the retired Phase-1 components and styles only after `grep -r` shows zero references; keep the `prune-unreferenced-images` hook and `scripts/serve-gzip.mjs`.
- **Acceptance:**
  - [x] Old collection URLs return 301 with the correct family target (verified in built output); sitemap covers every static route.
  - [x] Zero references to removed modules; build + `tsc` clean.

#### R6 — Redesign QA & Client Review

**Files:** update `docs/qa-report.md`.

- [x] Breakpoint sweep + tray usability on `/products` at 320/390/480/768/1024/1440 (headless Chrome, CDP — same harness as M6).
- [x] Re-run the M5 Lighthouse ≥90 gate (four combos) on the landing page + `/products`, on a quiet machine. *(Run on this machine 2026-09-05 — 100/98/100/100, gate met; numbers + report files in docs/qa-report.md.)*
...
7. **Real product photography:** 169 of the 258 products render the placeholder swatch at R1 build time (89 carry real photography; the 3 index-absent codes WG-37/WG-43/HGF-252 were excluded and quarantined in R1 — §10 Session 12). — **RESOLVED 2026-09-09:** the client supplied the full photography set; the catalogue is now **258/258 real, 0 placeholders**, 0 stray asset files (§10 Session 21).
...
- Milestone(s) worked on: **R5 — Redirects, SEO & Old-Page Teardown** (Phase 2) — complete. R4's Session-15 handoff ("next: R5") executed; implementation agent-written, then independently reviewed + verified here before closing.
- Completed this session:
  - **The redirect mechanism (agent's key call, verified in built output + over the Workers pipeline)**: Astro config `redirects` in `astro.config.mjs`, map generated at config load from `src/content/collections/*.json` slugs (`/collections/<slug>` → `/products/<slug>`) — the project-root-fs pattern already used for `__R3_CATALOGUE_PDF__`. `@astrojs/cloudflare`'s `astro:build:done` hook writes every `redirect` route from the manifest into `dist/client/_redirects` (Netlify-style asset rules via `@astrojs/underscore-redirects`, both trailing-slash variants), and Workers Static Assets serves them at the asset layer — a real HTTP 301, redirects in the asset bundle, no runtime map, no on-demand function. `build.redirects: false` stops Astro from *also* emitting 200-servable meta-refresh HTML stubs (a lost rule must 404 loudly, not soft-redirect). **The PRD's "repurpose [slug].astro" wasn't literally possible**: an existing page file outranks config redirects in route priority, so the page + the four showcase components were deleted instead — the outcome matches the milestone's intent.
  - **Deleted** (grep-proven zero references in src, then again in built HTML): `src/pages/collections/[slug].astro`, `AppShowcase.astro`, `CollectionSpread.astro`, `CollectionDialog.astro`, `IndexStrip.astro`. No orphaned global styles existed (all four components were scoped; tokens/global untouched).
  - **`src/pages/sitemap.xml.ts` rewritten**: exactly the 27 indexable routes (/, /products, 6 families, 19 series), derived from the collections collection + the SERIES taxonomy table (`data/catalogue-master.ts`) — the same derivation as the catalogue template's getStaticPaths, so the sitemap cannot drift from the route tree. Noindex trio (/request, /thank-you, /style-guide) stays excluded + noindexed; retired URLs never listed.
  - **`Nav.astro`**: family links (desktop + mobile) → `/products/[family]`; the 301s now exist only for legacy/inbound links. `style-guide.astro`: the IndexStrip/CollectionSpread demo sections replaced by a "Retired (R5)" note. Comment-only updates in `Seo.astro` + `content.config.ts` (the `secondaryApplicationImage` schema field now documents that no template renders it — the prune hook keeps those images out of the bundle).
  - **Bonus bug found + fixed**: series-page canonicals had pointed at their family URL — `headPath` in the catalogue template used `famHref` at every depth, so `/products/wood-grain/wg` canonicalized to `/products/wood-grain` (a duplicate-content smell the R5 sitemap work surfaced). Now depth-correct (`/products/wood-grain/wg` → itself).
  - Verification before closing (agent suite, then my independent pass): `npm run build` green + `npx tsc --noEmit` clean; **my checks** — `dist/client/_redirects` = exactly 12 rules (6 families × both slash forms), no `dist/client/collections/` dir (redirects: false held); sitemap 27 locs ↔ disk route set identical modulo the noindex trio; **12/12 old-URL variants → 301 with the correct `Location`** through a fresh `wrangler dev` (asset layer; `curl -sL` chain lands on `/products/<family>/` 200); unknown slugs (`/collections/nope`, `/collections/wood-grain/extra`, bare `/collections`) → loud 404s; zero `href="/collections` in any built HTML; canonicals depth-correct on /, family, and series pages (built HTML); noindex trio intact; smoke through the Workers pipeline — /, /products, a series page, /request, /style-guide all 200 with tiles, the tray `astro-island`, and the 258-key recap body present. (Stale R4-era `wrangler dev` on :4329 serving the old dist was killed; scratch `.wrangler/` state removed.)
- Decisions made (and why):
  - **Config `redirects` over HTML/meta-refresh stubs** — the only option yielding a true HTTP 301 in this stack; the adapter + Workers Static Assets support it natively and the rules ship inside the asset bundle exactly as the PRD requires ("verified in built output": the `_redirects` file is the artifact).
  - **`build.redirects: false`** — soft 200 stubs would mask a lost redirect rule; loud 404s are the correct failure mode for retired URLs.
  - **Deletion over literal repurposing** of `[slug].astro` — Astro route priority puts page files ahead of config redirects (documented in Astro); the config file is the clean single source. Logged in §5 R5.
  - The `secondaryApplicationImage` schema field stays (content-freeze: taxonomy/content schema untouched in R5; unused images never ship thanks to the prune hook) — a logged expectation, and cleanup can ride with M7's content freeze if the client agrees.
- Blocked on / open questions: unchanged — real photography for 169 placeholders (§7 Q7), catalogue PDF asset (§7 Q6 — CTA mechanism built + both-branch verified in Session 14, waiting on the file), trims scope (§7 Q5), domain / brand / real keys / GitHub remote + Workers Builds; live Resend delivery needs the client key.
- Next session should start with: **R6 — Redesign QA & Client Review** — the last Phase-2 milestone, with the human gates the loop stops at: update `docs/qa-report.md` (breakpoint sweep + tray usability on /products at 320–1440, headless Chrome CDP — same harness as M6); re-run the M5 Lighthouse ≥90 gate (four combos) on the landing page + /products; human visual pass at 1440/390 (settles M1/M2/M3 residuals); client review of the catalogue (taxonomy groupings, placeholder treatment, copy — including the Session-14/15 items flagged for sign-off: the range heading copy and the R4 stale-code interpretation); sign-off recorded in §10. The automated portion can run in-session; the visual pass and client review are human-only and must not be faked (§5 R6).

### Session 17 — 2026-09-05
- Milestone(s) worked on: **R6 — Redesign QA & Client Review** (Phase 2) — automated portion complete; **the two human gates remain open and the milestone is not claimed done**. R5's Session-16 handoff ("next: R6") executed in-session (no implementation agent — the R6 changes are verification + docs).
- Completed this session:
  - **Breakpoint + tray sweeps re-run on the R5 build** (d74c2df, `docs/qa-report.md` appended with the full R6 record): the R3 landing suite (17/17 — no h-scroll 320–1440, 6 family tiles → `/products/<family>` all 200, tray CTAs open/close) and the R2 catalogue suite (ALL PASS — `/products` 258 tiles + `/products/wood-grain/wg` at 6 widths, mobile filter disclosure, live search + empty + clear, tray add → header → Escape → re-open). Both still green after R5's teardown — the deleted components left no trace.
  - **M5 Lighthouse ≥90 gate re-run: MET — 100/98/100/100** (landing + /products × desktop/mobile; Lighthouse 13.4.1, headless Chrome, report JSONs in /tmp). The historical mobile straddle (Session 9: 85–98) was simulator variance — today's mobiles sit 98/100 with no structural change since. Note for future runs: Lighthouse 13 dropped `--preset=mobile` (mobile is the default; desktop via `--preset=desktop`).
  - **`docs/qa-report.md`**: R6 section appended — what was verified, the 4-combo table, how to re-run.
- Decisions made (and why):
  - **The Lighthouse gate was run on this machine, not a "quiet machine"** — the R6 text asks for a quiet-machine run because of the M5 variance; the result (98/100 mobiles, 100/100 desktops) is at the top of the historical band and the acceptance clause allows the variance logged. The run is real (not claimed); if the client demo needs an independent quiet-machine run it can ride with the human visual pass.
  - **R6 not marked done** — two acceptance bullets are human-only by design: the 1440/390 visual mockup pass and client review/sign-off. Nothing further is automatable; the loop stops here and hands those gates to the humans.
- Blocked on / open questions — **the R6 human gates, plus the standing client items**: §7 Q5 (trims scope), Q6 (catalogue PDF asset — CTA mechanism built, waiting on the file), Q7 (real photography for 169 placeholders); domain/brand/real keys; GitHub remote + Workers Builds; live Resend delivery. Flagged-for-sign-off items to bring into the client review: the range heading copy ("The complete LAMINA range." — Session 14) and the R4 stale/unknown-code interpretation (Session 15). Browser matrix rows (Safari/Firefox/Edge/iOS/Android) also still need human/device access (M6).
- Next session should start with: **the R6 human gates** — the 1440/390 visual pass vs `reference/lamina-design-direction.html` (landing + catalogue + tray; settles M1/M2/M3 residuals), then the client review of the catalogue with the flagged items above; record the outcome in §10 and mark R6 done only when both are real. After R6: M7 (content freeze & client review folds in), M8 (launch — only when the client asks, incl. domain/keys), M9 (handover).

### Session 18 — 2026-09-05
- Milestone(s) worked on: **R6 (client feedback on the R3 landing)** — the "Download the Catalog" hero button and the statement-row PDF action were missing; restored.
- Completed this session:
  - **Root cause**: not a code deletion — R3's build-time gate. Both PDF CTAs render only when `public/catalogue.pdf` exists at build start (`__R3_CATALOGUE_PDF__`), and no such file has ever been committed (git history empty for it). The client's catalogue **`lamina-wall.pdf`** (12.9 MB, valid PDF 1.7, 119 pages — the same source document M2's imagery came from) has sat at the repo root, tracked, since Aug 24 — outside the gate's view. Pre-R3 the hero rendered the PDF button unconditionally (a46c4fe baseline), so the gating read as a regression against the approved mockup ("Download full catalogue (PDF)" in the CTA row).
  - **Fix**: copied `lamina-wall.pdf` → `public/catalogue.pdf` — the mechanism's designed end-state, zero code change to the gate — and added the missing `download` attribute to the hero's PDF anchor (`Hero.astro`; the statement CTA had it since R3, the hero's pre-R3 baseline never did), so both actions save the file rather than navigate.
  - Verification: `npm run build` green + `npx tsc --noEmit` clean; built landing carries **both** CTAs (`<a href="/catalogue.pdf" download>` in the hero `.btn-outline` pair and the statement `.btn` row); `dist/client/catalogue.pdf` present at 12,899,465 bytes and serves 200 over the gzip server; landing CDP suite ALL PASS with the PDF assertion updated to expect the 2 CTAs (and the harness now clears the Chrome HTTP cache — a stale-profile false failure cost one cycle). `docs/qa-report.md` R6 section annotated.
- Decisions made (and why):
  - **Ship the client's tracked file rather than a new copy from elsewhere** — it is the only catalogue PDF in the repo and the one the client committed; §7 Q6's "which file" is answered by the client's own delivery. If a different final file is intended, replacing `public/catalogue.pdf` + rebuild is the whole change.
...
### Session 21 — 2026-09-09
- Milestone(s) worked on: **R1 data backfill** (photography for all 258 codes) — the "169 placeholders" item is closed; **§0 R1 note, §3.4 snapshot, §7 Q7 updated**.
- Completed this session:
  - Client uploaded the full swatch photography set into `src/assets/products/<family>/`; regenerated the catalogue from the master. Result: **258 real / 0 placeholder** (was 89/169), zero broken `swatchImage` refs, build + `tsc` clean.
  - **AMF images were in `src/assets/products/metal/`** but the taxonomy assigns AMF to `glossy-decorative` (Decorative & Mirror). Moved the 20 `amf-*.png` to `glossy-decorative/` so the generator finds them. Regeneration also **fixed the stale AMF product JSONs**: 3 had `collection: "metal"` and 17 had `collection: "silver"` (an invalid slug that would fail the build) — all now `"glossy-decorative"`, matching the master.
  - **Generator data-loss guard bug fixed** (`scripts/generate-catalogue.mjs`): `previouslyReal` matched *any* existing `swatchImage` path, and the shared placeholder ref exists on disk — so every placeholder product counted as "previously real" and any run ending with a placeholder aborted with a false "real image lost" error. Guard now excludes `PLACEHOLDER_REF`. (This was the first run with remaining placeholders since the guard shipped, so it had never surfaced.)
  - **Alt text for the 167 newly-real products** set from each product's own printed name in the established pattern (`"{material} panel swatch in {Name}."`; materials per series: Wood-grain / Marble-effect / High-gloss marble-effect / Metal / Fabric-wrapped / Matte solid-colour / High-gloss solid-colour / High-gloss decorative). Rationale: §3.3 requires real alt on every image, and the image-derived tones of the original 89 can't be reproduced without inspecting each swatch; the printed name is the truthful, non-invented descriptor. The generator preserves these on subsequent runs.
  - **HGM-284A/B correction (client decision):** the manufacturer's index prints the Kanas Slate pair as HGM-285A/B, but the client's uploaded photography is numbered 284A/B and the client confirmed 284A/B are correct. The client hand-added `hgm-284a.json`/`hgm-284b.json`; I fixed both (they had `code: "HGM-284A"` twice, `hgm-284b` pointed at `hgm-284a.png`, and one alt read "Athena Slate"), updated the master's HGM-285 row → HGM-284 (`data/catalogue-master.ts`), and deleted the superseded `hgm-285a.json`/`hgm-285b.json`. `subcategory-taxonomy.md` left untouched (extraction record). HGM series stays 18; stone-marble stays 50.
- Decisions made (and why):
  - **AMF images relocated to match the taxonomy family**, rather than relabeling AMF as Metal — `subcategory-taxonomy.md`/master are the source of truth (§AGENTS.md hard constraint).
  - **Newly-real products got alt text only** (no description/tags/featured) — those fields stay optional and curated; alt is the only schema-required field.
  - HGM-284/285 is a documented divergence between the taxonomy and the physical catalogue, resolved by the client — logged here and in the master, not silently.
- Blocked on / open questions:
  - **`src/assets/products/wood-grain/wg-fffff`** — a stray 1100×296 PNG swatch with a mangled filename; matched no catalogue code and was not a duplicate of any existing swatch (likely WG-37 or WG-43, the quarantined index-absent codes). The client reviewed it and deleted it; the assets folder is now exactly 258 files, one per catalogue code.
  - Unchanged: R6 human gates (visual pass + client sign-off), §7 Q8 (Phase-3 copy — blocks S2/S3/S5/S6), §7 Q9 (imagery), domain/brand/real keys, GitHub remote + Workers Builds, live Resend delivery.
- Next session should start with: **S2 — Landing: Features/Advantages section** — blocked until the client supplies the page-4 copy (§7 Q8); otherwise pick up any non-blocked Phase-3 item.
